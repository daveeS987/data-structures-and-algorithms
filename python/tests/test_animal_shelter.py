...
from animal_shelter.animal_shelter import Animal_Shelter, Animal


def test_can_successfully_instantiate_an_empty_Animal():
    animal1 = Animal()
    assert animal1.value == None
    assert animal1.next == None
...
def test_can_successfully_enqueue_one_animal_into_animal_shelter():
    ...
    assert new_shelter.front.value == "dog"
    assert new_shelter.rear.value == "dog"
    assert new_shelter.length == 1


def test_can_enqueue_multiple_animals_into_Animal_shelter(new_shelter):
    assert new_shelter.front.value == "dog"
    assert new_shelter.rear.value == "cat"
...
def test_can_successfully_dequeue_pref_animal_at_the_end(cat_last):
    ...
    expected = "cat"
    assert actual == expected
    assert cat_last.front.value == "dog"
    assert cat_last.rear.value == "dog"
